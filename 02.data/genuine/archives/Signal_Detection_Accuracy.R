...
data_happy <- dataset %>%
  AccuracyDetection("happiness")

data_fear <- dataset %>%
  AccuracyDetection("fear")

data_anger <- dataset %>%
  AccuracyDetection("anger")

data <- rbind(data_happy, data_fear, data_anger) %>%
...
# Model Fitting
anova_d <- aov_ez("participant", "d_prime",data = data ,within = c("Emotion","Session","Elicitation"), between = "Group")
...
# plots
data_mean <- data %>%
  group_by(Emotion, Session, Group, Elicitation) %>%
